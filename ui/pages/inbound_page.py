...
class InboundPage(ctk.CTkFrame):
    ...
    def create_form(self):
        ctk.CTkLabel(self.form_card, text="入庫登錄", font=Font.SUBTITLE, text_color=Color.TEXT_DARK).pack(anchor="w", padx=Layout.CARD_PADDING, pady=(Layout.CARD_PADDING, 10))
        ...
        ctk.CTkLabel(date_wrapper, text="有效期限", font=Font.BODY, text_color=Color.TEXT_DARK).pack(anchor="w", pady=(0, 5))
        
        d_box = ctk.CTkFrame(date_wrapper, fg_color="transparent")
        d_box.pack(fill="x")
        
        current_year = datetime.now().year
        self.combo_year = ctk.CTkComboBox(d_box, values=[str(y) for y in range(current_year, current_year+10)], width=80, height=35)
        self.combo_year.pack(side="left", padx=(0, 5))
        self.combo_year.set(str(current_year))
        
        self.combo_month = ctk.CTkComboBox(d_box, values=[f"{m:02d}" for m in range(1, 13)], width=70, height=35)
        self.combo_month.pack(side="left", padx=5)
        self.combo_month.set(datetime.now().strftime("%m"))
        
        self.combo_day = ctk.CTkComboBox(d_box, values=[f"{d:02d}" for d in range(1, 32)], width=70, height=35)
        ...
        ctk.CTkLabel(note_wrapper, text="備註", font=Font.BODY, text_color=Color.TEXT_DARK).pack(anchor="w", pady=(0, 5))
        self.entry_note = ctk.CTkEntry(note_wrapper, height=35)
        self.entry_note.pack(fill="x")

        # 按鈕區 (Row 2)
        btn_row = ctk.CTkFrame(content, fg_color="transparent")
        btn_row.grid(row=2, column=0, columnspan=4, pady=(10, 0), sticky="e")
        self.btn_submit = ctk.CTkButton(btn_row, text="確認入庫", fg_color=Color.PRIMARY, width=140, height=38, command=self.handle_submit)
    ...
    def handle_submit(self):
        ...
        y = self.combo_year.get(); m = self.combo_month.get(); d = self.combo_day.get(); expiry = f"{y}-{m}-{d}"
        ...
        try:
            ...
            qty = float(qty_str)
            if price_str: unit_price = float(price_str)
        ...
        if success:
            self.entry_qty.delete(0, "end"); self.entry_price.delete(0, "end"); self.entry_batch.delete(0, "end"); self.entry_note.delete(0, "end")
