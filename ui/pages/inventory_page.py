...
class InventoryPage(ctk.CTkFrame):
    ...
    def create_form(self):
        ctk.CTkLabel(self.form_card, text="庫存手動調整 / 盤點 / 領料", font=Font.SUBTITLE, text_color=Color.TEXT_DARK).pack(anchor="w", padx=20, pady=(15, 5))
        ...
        ctk.CTkLabel(content, text="2. 原料", font=Font.BODY, text_color=Color.TEXT_DARK).grid(row=0, column=1, padx=10, pady=5, sticky="w")
        self.combo_material = ctk.CTkComboBox(content, state="readonly", command=self.on_material_selected)
        self.combo_material.set("請先選類別")
        self.combo_material.grid(row=1, column=1, padx=10, pady=(0, 10), sticky="ew")

        self.lbl_current_stock = ctk.CTkLabel(content, text="目前庫存: --", text_color=Color.PRIMARY, font=("Arial", 14, "bold"))
        self.lbl_current_stock.grid(row=1, column=2, padx=10, pady=(0, 10), sticky="w")
        
        ctk.CTkLabel(content, text="動作類型", font=Font.BODY, text_color=Color.TEXT_DARK).grid(row=0, column=3, padx=10, pady=5, sticky="w")
        self.combo_action = ctk.CTkComboBox(content, state="readonly", values=["領用/消耗 (-)", "盤點盤虧 (-)", "報廢 (-)", "盤點盤盈 (+)", "其他增加 (+)"])
        self.combo_action.set("領用/消耗 (-)")
        self.combo_action.grid(row=1, column=3, padx=10, pady=(0, 10), sticky="ew")

        # 第二排
        ctk.CTkLabel(content, text="變動數量 (正數)", font=Font.BODY, text_color=Color.TEXT_DARK).grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.entry_qty = ctk.CTkEntry(content, placeholder_text="例如: 1")
        self.entry_qty.grid(row=3, column=0, padx=10, pady=(0, 10), sticky="ew")

        ctk.CTkLabel(content, text="備註", font=Font.BODY, text_color=Color.TEXT_DARK).grid(row=2, column=1, columnspan=2, padx=10, pady=5, sticky="w")
        self.entry_note = ctk.CTkEntry(content, placeholder_text="說明原因")
        self.entry_note.grid(row=3, column=1, columnspan=2, padx=10, pady=(0, 10), sticky="ew")
        ...
        self.btn_submit.grid(row=3, column=3, padx=10, pady=(0, 10), sticky="e")
