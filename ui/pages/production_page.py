...
class ProductionPage(ctk.CTkFrame):
    ...
    # ... (refresh_data, on_product_selected, handle_submit 保持不變) ...
    def refresh_data(self):
        ...
        now = datetime.now(); self.combo_year.set(str(now.year)); self.combo_month.set(f"{now.month:02d}"); self.combo_day.set(f"{now.day:02d}")
    ...
    def on_product_selected(self, val):
        ...
        try:
            ...
            if days:
                ...
                self.combo_year.set(str(target_date.year)); self.combo_month.set(f"{target_date.month:02d}"); self.combo_day.set(f"{target_date.day:02d}")
    ...
    def handle_submit(self):
        ...
        if success:
            ...
            self.refresh_data(); self.on_product_selected(prod_str)
